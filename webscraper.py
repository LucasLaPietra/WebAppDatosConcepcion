...
def mapMonth(month,year):
    ...
    driver.get(f'https://cdeluruguay.gob.ar/datagov/proveedoresContratadosAMR.php?anio={year}&mes={month}')
    content = driver.page_source
    soup = BeautifulSoup(content,features="lxml")
    for row in soup.findAll('tr', attrs={'class':'textoTabla'}):
...
def writeLastRun():
    driver.get(f'https://cdeluruguay.gob.ar/datagov/proveedoresContratados.php')
    content = driver.page_source
    soup = BeautifulSoup(content,features="lxml")
